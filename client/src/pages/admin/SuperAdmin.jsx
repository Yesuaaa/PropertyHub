...
import { validateRegistration, cleanText } from '../../utils/validation';
...
export default function SuperAdmin() {
    ...
    (async () => {
      ...
      try {
        const [usersRes, ticketsRes, logsRes] = await Promise.all([
          ...
          api.get('/superadmin/activity-logs')
        ]);
        if (!cancelled) {
  ...
  const handleDeleteUser = async (id) => {
    if (!window.confirm(`Delete user #${id}? This action cannot be undone.`)) return;
    try {
  ...
  const handleToggleActive = async (id) => {
    if (!window.confirm(`Change active status for user #${id}?`)) return;
    try {
  ...
  const handleRoleChange = async (id, role) => {
    if (!window.confirm(`Change user #${id} role to ${role}?`)) return;
    try {
                ...
                <tbody className="divide-y divide-[#8fa3b0]/15">
                  {filteredTickets.map((t) => (
                    <tr key={t.id} className="hover:bg-[#8fa3b0]/5 transition-colors">
                      ...
                      <td className="px-5 py-4 text-xs text-[#8fa3b0] font-mono">{new Date(t.created_at).toLocaleString(undefined, { dateStyle: 'medium', timeStyle: 'short' })}</td>
                      <td className="px-5 py-4">
        ...
        <>
          {logs.length === 0 ? (
            <div className="border-2 border-[#8fa3b0]/20 p-12 text-center">
              <p className="text-sm text-[#5a6d78]">No activity logs found. Run migration 009_create_activity_logs.sql if this is the first deployment.</p>
            </div>
          ) : (
            <div className="border-2 border-[#1a1a1a] overflow-x-auto">
              <table className="w-full text-left min-w-[900px]">
                <thead>
                  <tr className="border-b-2 border-[#1a1a1a]">
                    <th className="px-5 py-3 text-[10px] font-mono font-semibold tracking-[0.2em] uppercase text-[#8fa3b0]">Date</th>
                    <th className="px-5 py-3 text-[10px] font-mono font-semibold tracking-[0.2em] uppercase text-[#8fa3b0]">Actor</th>
                    <th className="px-5 py-3 text-[10px] font-mono font-semibold tracking-[0.2em] uppercase text-[#8fa3b0]">Action</th>
                    <th className="px-5 py-3 text-[10px] font-mono font-semibold tracking-[0.2em] uppercase text-[#8fa3b0]">Target</th>
                    <th className="px-5 py-3 text-[10px] font-mono font-semibold tracking-[0.2em] uppercase text-[#8fa3b0]">Change</th>
                  </tr>
                </thead>
                <tbody className="divide-y divide-[#8fa3b0]/15">
                  {logs.map((log) => (
                    <tr key={log.id} className="hover:bg-[#8fa3b0]/5 transition-colors">
                      ...
                      <td className="px-5 py-4 text-sm text-[#1a1a1a] font-medium">
                        ...
                        {log.actor_email && <div className="text-[10px] text-[#8fa3b0] font-mono mt-1">{log.actor_email}</div>}
                      </td>
                      <td className="px-5 py-4 text-xs font-mono uppercase tracking-[0.1em] text-[#5a6d78]">{log.action?.replaceAll('_', ' ')}</td>
                      <td className="px-5 py-4 text-sm text-[#5a6d78]">
                        {log.ticket_id ? `Ticket #${log.ticket_id}` : (log.target_name || log.target_email || '—')}
                      </td>
                      <td className="px-5 py-4 text-xs font-mono text-[#5a6d78]">
