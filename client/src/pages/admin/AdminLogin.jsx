...
import { validateLogin, cleanText } from '../../utils/validation';
...
export default function AdminLogin() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const user = await login(cleanText(email).toLowerCase(), password);
  ...
  return (
    <div className="min-h-screen flex items-center justify-center py-12 px-4">
      <div className="max-w-md w-full">
        <div className="border-2 border-[#1a1a1a] bg-[#f5f3ef] p-6 sm:p-10 relative">
          ...
          <form onSubmit={handleSubmit} className="space-y-5">
            ...
            <button
              type="submit"
              disabled={loading}
              className="w-full text-[#f5f3ef] py-3 text-xs font-semibold tracking-[0.15em] uppercase bg-[#e05a30] border-2 border-[#e05a30] hover:bg-[#c44a25] hover:border-[#c44a25] transition-all duration-200 mt-2 cursor-pointer disabled:opacity-50 disabled:cursor-not-allowed"
            >
